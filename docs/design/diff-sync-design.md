...
- **Multi-star filter**: 共演/omnibus titles are dropped — `IJavTorrentExtractor` counts each card's `/actress/` links into `star_count` (solo = 1), and anything above 1 is filtered (`_drop_multi_star`). RSS-supplement items have `star_count=0` and pass through (omnibus titles rarely mention the star, so the noise filter catches most of them)
