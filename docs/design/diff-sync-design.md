...
- **Visibility filter** (`scrapers/v2/filters.py`, applied in `fetch_star`): VR and 共演/omnibus titles are dropped at collection time and never enter the DB — `star_count > 1` (ijav card actress-link count, solo = 1), 【VR】/`[VR]` in the title, VR resolution tags, VR-only label code prefixes (SIVR/KAVR/MDVR/…, catches legacy rows with blank title/resolution), 共演/オムニバス keywords, Latin/JP cast-list title patterns, and titles mentioning another rostered star (the synced star's own aliases are excluded — uploaders routinely tag torrents with her name). Dropped codes are logged with their reason. Existing rows from before the filter: `scripts/drop_hidden_titles.py` (offline, same rules)
...
| **Cover download failure / unusable cover** | `upsert_title` and `write_batch` preserve existing `cover_b64` on conflict; a **new** title without a usable cover is skipped entirely (stays "new", retries next sync). Covers taller than wide (`COVER_MAX_HW_RATIO = 1.0` in `sinks.py`) are vertical front-cover thumbnails — the blob is dropped and the title is treated as coverless |
...
| **Titles without a valid magnet** | `write_batch` skips items with no candidates whose btih hash extracts to 40 hex chars; such rows are never recorded (unplayable). `scripts/cleanup_bad_titles.py` applies the same rule offline to legacy rows |
| **Repeatedly sink-rejected titles** | Skips (`no_magnet` / `no_cover` / `bad_cover`) are upserted into `title_blacklist`; `_drop_blacklisted` filters them before the diff in later syncs so their covers are never re-downloaded. Entries older than 7 days get one retry pass; a successful write clears the entry (`clear_blacklist_entries`) |
| **Stars with persistently empty RSS results** | Obscure names sukebei doesn't have used to cost 3 retries × N query variants every sync and spam the error log. `NoRssResultsError` (a distinct `IncompletePageError` subclass) is counted into `star_rss_state`; after 3 consecutive empty syncs phase B skips the star for 7 days (`load_rss_suppressed_stars`), and any non-empty result resets the streak (`clear_rss_empty`) |
