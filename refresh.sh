...
# 流程:
#   1. search-news.py → PostgreSQL (ijavtorrent 抓取)
#   2. fetch-jable.py → PostgreSQL (jable 封面+m3u8)
#   3. fetch-social.py → PostgreSQL (X/Twitter 最新动态)
#
# 前端通过 /api/stars 实时读取数据库，无需生成静态 HTML。
...
# Step 1: 抓取 ijavtorrent 数据 → PostgreSQL
