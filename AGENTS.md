...
- **On-demand download discipline (不看不下、看哪下哪)**: locked by `tests/test_torrent_engine_arch.py::TestOnDemandDownloadDiscipline` (mock, fast) and `tests/test_ondemand_download.py` (live local seed). Contract: add = paused + all-zero priorities; play/resume/progress = urgent(7) only inside ±30 window + moov, verified retained(1), everything else stop(0); seek = ±15 window, abandoned pieces drop to 0; range requests raise only their own ±2 pieces; `set_sequential_download(True)` is never allowed.
